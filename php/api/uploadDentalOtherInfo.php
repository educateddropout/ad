...
$fileDirectory = "/uploads/patientDentalOtherFiles/";
$uploadCtr = 0; // okay to upload
	...
	$results = $database->countPdfFiles($patientId);
	...
	if($imageFileType != "pdf" ) {
	    ...
	    array_push($messages, "Sorry, only PDF files are allowed.");
	...
	if ($uploadCtr == 1) {
	    ...
	    if (move_uploaded_file($fileTmpName, SITE_ROOT.$fileName)) {
	        ...
	        $database->savePatientPdfFile($patientId, $_FILES["file"]["name"], $fileName, $userId, $pdfFileName);
